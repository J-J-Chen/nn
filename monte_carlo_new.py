...
dimensions = [1,2,3,4,5,10,20,30,40]
num_reruns = [10,10,10,10,10,5,5,5,5]
...
for i in range(1,100000):
    if os.path.isfile("mc_trap_runs_std/points_" + str(sample_points[0]) + "_run_" + str(i)):
        run += 1
    else:
        run = i
        break
...
for asdf,sample_point in enumerate(sample_points):
    ...
    f = open("mc_trap_runs_std/points_" + str(sample_point) + "_run_" + str(run), "a")
    f.write("dim,sample_points,num_reruns,mc_ans,mc_ans_std,mc_1000_time,mc_time_std,trap_ans,trap_ans_std,trap_1000_time,trap_time_std\n")
    for j, dim in enumerate(dimensions):
        ...
        mc_ans = np.empty(num_reruns[j])
        mc_times = np.empty(num_reruns[j])
        for cur_rerun, num_rerun in enumerate(range(num_reruns[j])):
            mc_start_time = time.perf_counter()
            mc_ans[cur_rerun] = mc.integrate(some_function, dim=dim, N=sample_point_dim, integration_domain=int_domain)
            mc_end_time = time.perf_counter()
            mc_times[cur_rerun] = (mc_end_time-mc_start_time)
        mc_final_ans = mc_ans.mean()
        mc_ans_std = np.std(mc_ans)
        mc_time_std = np.std(mc_times)
        mc_time = 1000*mc_times.mean()

        trap_ans = np.empty(num_reruns[j])
        trap_times = np.empty(num_reruns[j])
        for cur_rerun, num_rerun in enumerate(range(num_reruns[j])):
            trap_start_time = time.perf_counter()
            trap_ans[cur_rerun] = (trap.integrate(some_function, dim=dim, N=sample_point_dim, integration_domain=int_domain))
            trap_end_time = time.perf_counter()
            trap_times[cur_rerun] = (trap_end_time-trap_start_time)
        trap_final_ans = trap_ans.mean()
        trap_ans_std = np.std(trap_ans)
        trap_time_std = np.std(trap_times)
        trap_time = 1000*trap_times.mean()
        f.write(str(dim)+","+str(sample_point_dim)+","+str(num_reruns[j])+","+str("%.6f"%mc_final_ans)+","+str("%.6f"%mc_ans_std)+","+str("%.6f"%mc_time)+","+str("%.6f"%mc_time_std)+","+str("%.6f"%trap_final_ans)+","+str("%.6f"%trap_ans_std)+","+str("%.6f"%trap_time)+","+str("%.6f"%trap_ans_std)+"\n")
